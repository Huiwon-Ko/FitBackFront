...
const NaverButton = ({ setGetToken, setUserInfo }) => {
      
  
	const { naver } = window
	const NAVER_CLIENT_ID = 'oeiI6UyzGUxfwwnLq7_l' // 네이버 Client ID 
	const NAVER_CALLBACK_URL = 'http://localhost:8080/login/oauth2/code/naver' // 네이버 Callback URL 
	...
	const initializeNaverLogin = () => {
		const naverLogin = new naver.LoginWithNaverId({
			...
			isPopup: true, // 팝업창 여부
			loginButton: { color: 'green', type: 3, height: 45 }, // 버튼 타입 ( 색상, 타입, 크기 )
			callbackHandle: true,
		})
		naverLogin.init()
